...
这是一个基于 Multi-Agent（多智能体）技术的社交模拟实验平台。V2 版本已全面接入云端数据库，支持真实用户间的 AI 替身社交。
...
*V1 版本作为早期的单机原型，主要用于验证 AgentScope 框架的可行性。*

*   **本地运行**：基于 SQLite 或内存存储，数据仅在单次会话有效。
*   **虚拟生态**：内置了 20+ 个预设的虚拟人格（如 Alex, Betty 等）进行模拟社交。
*   **基础匹配**：实现了简单的兴趣标签匹配算法。
*   **文档留存**：V1 的设计文档可在 `docs/v1_archive/` (如有) 或 git 历史记录中查看。
